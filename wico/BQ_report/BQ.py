...
def generate_dashboard_layout(target_date_str):
    ...
    red_palette = ['#2C0000', '#FF6B6B', '#8B0000', '#FF4500', '#4B0082', '#FF0000', '#800080']
    ...
    if not df_volume.empty:
        ...
        fig_vol.update_traces(
            ...
            textfont=dict(size=20, family='Arial Black', color='white'),
            cliponaxis=False
        )
    ...
    return dbc.Container([
        ...
        dbc.Row([
            ...
            dbc.Col(
                ...
                width=6, # 占据一半宽度
                # 【修改点】高度设为 820px，大致等于右侧两个图表的高度之和
                style={'height': '820px'} 
            ),
            ...
            dbc.Col([
                # 右上：不良统计（按产品类型）
                dbc.Row([
                    dbc.Col(create_graph_card(fig_dist1), style={'height': '400px'})
                ], className="mb-3"), # mb-3 增加下方间距
                
                # 右下：客户产量
                dbc.Row([
                    dbc.Col(create_graph_card(fig_vol), style={'height': '400px'})
                ])
            ], width=6) # 占据另一半宽度
