#!/usr/bin/python
# -*- coding: UTF-8 -*-
import sqlite3
import datetime
import os
import sys
import pprint
import mysql.connector as mariadb
import traceback
from pathlib import Path
from kivy.logger import Logger
from contextlib import closing


if getattr(sys, "frozen", False):  # bundle mode with PyInstaller
    os.environ["WICO_ROOT"] = sys._MEIPASS
else:
    os.environ["WICO_ROOT"] = str(Path(__file__).parent)

KV_DIR = f"{os.environ['WICO_ROOT']}"


def get_sqlite_connection():
    """获取SQLite连接"""
    return sqlite3.connect(f"{os.environ['WICO_ROOT']}/db.db")

def get_mariadb_connection():
    """获取MariaDB连接"""
    try:
        return mariadb.connect(
            user="imasenwh",
            password="596bf648aa7f80d8",
            host="mysql.sqlpub.com",
            port=3306,
            database="custom_feedback"
        )
    except Exception:
        print(traceback.format_exc())
        return None

def sync_ngrecord(sqlite_conn, mariadb_conn):
    with closing(sqlite_conn.cursor()) as sqlite_cursor:
        sqlcmd='''
        SELECT
        *
        FROM
        ngrecord
        WHERE Sync = "{}"
        '''.format(0)
        sqlite_cursor.execute(sqlcmd)
        values = sqlite_cursor.fetchall()
        
        if len(values) > 0:
            with closing(mariadb_conn.cursor()) as mariadb_cursor:
                for value in values:
                    # 把元组修改为list
                    value = list(value)
                    # 同步标识置1
                    value[-1] = 1
                    value = tuple(value)
                    
                    # 提交数据到服务器
                    sqlcmd = "INSERT INTO ngrecord VALUES {}".format(value)
                    sqlcmd = sqlcmd.replace("None", "NULL")
                    mariadb_cursor.execute(sqlcmd)

                    # 修改本地数据库的同步标识
                    sqlcmd = "REPLACE INTO ngrecord VALUES {}".format(value)
                    sqlcmd = sqlcmd.replace("None", "NULL")
                    sqlite_cursor.execute(sqlcmd)

                mariadb_conn.commit()
            sqlite_conn.commit()

def sync_volume(sqlite_conn, mariadb_conn):
    with closing(sqlite_conn.cursor()) as sqlite_cursor:
        sqlcmd='''
        SELECT
        *
        FROM
        volume
        WHERE Sync = "{}"
        '''.format(0)
        sqlite_cursor.execute(sqlcmd)
        values = sqlite_cursor.fetchall()
        
        if len(values) > 0:
            with closing(mariadb_conn.cursor()) as mariadb_cursor:
                for value in values:
                    # 把元组修改为list
                    value = list(value)
                    # 同步标识置1
                    value[-1] = 1
                    value = tuple(value)
                    
                    # 提交数据到服务器
                    sqlcmd = "REPLACE INTO volume VALUES {}".format(value)
                    sqlcmd = sqlcmd.replace("None", "NULL")
                    mariadb_cursor.execute(sqlcmd)

                    # 修改本地数据库的同步标识
                    sqlcmd = "REPLACE INTO volume VALUES {}".format(value)
                    sqlcmd = sqlcmd.replace("None", "NULL")
                    sqlite_cursor.execute(sqlcmd)
                
                mariadb_conn.commit()
            sqlite_conn.commit()

def sync_partlist(sqlite_conn, mariadb_conn):
    with closing(mariadb_conn.cursor()) as mariadb_cursor:
        sqlcmd='''
        SELECT
        *
        FROM
        partlist
        WHERE Sync = "{}"
        '''.format(0)
        mariadb_cursor.execute(sqlcmd)
        values = mariadb_cursor.fetchall()
        
        if len(values) > 0:
            with closing(sqlite_conn.cursor()) as sqlite_cursor:
                for value in values:
                    # 把元组修改为list
                    value = list(value)
                    # 同步标识置1
                    value[-1] = 1
                    value = tuple(value)
                    
                    # 更新本地数据库
                    sqlcmd = "REPLACE INTO partlist VALUES {}".format(value)
                    sqlcmd = sqlcmd.replace("None", "NULL")
                    sqlite_cursor.execute(sqlcmd)
                
                sqlite_conn.commit()
            mariadb_conn.commit()

def sync_seatlist(sqlite_conn, mariadb_conn):
    with closing(mariadb_conn.cursor()) as mariadb_cursor:
        sqlcmd='''
        SELECT
        *
        FROM
        seatlist
        WHERE Sync = "{}"
        '''.format(0)
        mariadb_cursor.execute(sqlcmd)
        values = mariadb_cursor.fetchall()
        
        if len(values) > 0:
            with closing(sqlite_conn.cursor()) as sqlite_cursor:
                for value in values:
                    # 把元组修改为list
                    value = list(value)
                    # 同步标识置1
                    value[-1] = 1
                    value = tuple(value)
                    
                    # 更新本地数据库
                    sqlcmd = "REPLACE INTO seatlist VALUES {}".format(value)
                    sqlcmd = sqlcmd.replace("None", "NULL")
                    sqlite_cursor.execute(sqlcmd)  # 修正这里：原来是 sql，应该是 sqlcmd
                
                sqlite_conn.commit()
            mariadb_conn.commit()

def sync_ngtype(sqlite_conn, mariadb_conn):
    with closing(mariadb_conn.cursor()) as mariadb_cursor:
        sqlcmd='''
        SELECT
        *
        FROM
        ngtype
        WHERE Sync = "{}"
        '''.format(0)
        mariadb_cursor.execute(sqlcmd)
        values = mariadb_cursor.fetchall()
        
        if len(values) > 0:
            with closing(sqlite_conn.cursor()) as sqlite_cursor:
                for value in values:
                    # 把元组修改为list
                    value = list(value)
                    # 同步标识置1
                    value[-1] = 1
                    value = tuple(value)
                    
                    # 更新本地数据库
                    sqlcmd = "REPLACE INTO ngtype VALUES {}".format(value)
                    sqlcmd = sqlcmd.replace("None", "NULL")
                    sqlite_cursor.execute(sqlcmd)
                
                sqlite_conn.commit()
            mariadb_conn.commit()

def sync_all():
    try:
        with closing(get_sqlite_connection()) as sqlite_conn, \
             closing(get_mariadb_connection()) as mariadb_conn:
            
            if mariadb_conn is None:
                return False
                
            t1 = datetime.datetime.now()
            sync_ngrecord(sqlite_conn, mariadb_conn)
            t2 = datetime.datetime.now()
            sync_volume(sqlite_conn, mariadb_conn)
            t3 = datetime.datetime.now()
            sync_partlist(sqlite_conn, mariadb_conn)
            t4 = datetime.datetime.now()
            sync_seatlist(sqlite_conn, mariadb_conn)
            t5 = datetime.datetime.now()
            sync_ngtype(sqlite_conn, mariadb_conn)
            t6 = datetime.datetime.now()
            
            Logger.info("ngrecord:{},volume:{},partlist:{},seatlist:{},ngtype:{}".format(
                t2-t1, t3-t2, t4-t3, t5-t4, t6-t5))
            return True
    except Exception:
        print(traceback.format_exc())
        return False

def sync_ngrecord_volume():
    try:
        with closing(get_sqlite_connection()) as sqlite_conn, \
             closing(get_mariadb_connection()) as mariadb_conn:
            
            if mariadb_conn is None:
                return False
                
            sync_ngrecord(sqlite_conn, mariadb_conn)
            sync_volume(sqlite_conn, mariadb_conn)
            return True
    except Exception:
        print(traceback.format_exc())
        return False
    
def overlap():
    with closing(get_sqlite_connection()) as sqlite_conn:
        cursor = sqlite_conn.cursor()
        cursor.execute("delete from partlist")
        cursor.execute("delete from seatlist")
        cursor.execute("delete from ngtype")
        sqlite_conn.commit()
    
    return sync_all()

if __name__=="__main__":
    a = sync_all()
    print(a)
